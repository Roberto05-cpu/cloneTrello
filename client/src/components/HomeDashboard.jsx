...
const HomeDashboard = () => {
  ...
  const handleCreateBoard = (e) => {
    e.preventDefault();
    console.log("Créer un nouveau board avec le titre :", titleBoard);

    axios
      .post(
        "http://localhost:3000/board/create-board",
        { title: titleBoard },
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
    ...
    <div className="m-5 max-h-screen overflow-y-auto pb-10">
      ...
      <div className=" mt-5 max-w-[2000px] overflow-x-auto flex items-center gap-5">
        <div className="w-[300px] h-[150px] bg-blue-200 rounded-[10px] hover:bg-blue-500 hover:text-white ">
          ...
          <p className="text-center mt-5 font-bold text-3xl">
            {statsOverview.totalBoards}
          </p>
        </div>
        <div className="w-[300px] h-[150px] bg-blue-200 rounded-[10px] hover:bg-blue-500 hover:text-white">
          <div className="p-3 flex items-center justify-between">
