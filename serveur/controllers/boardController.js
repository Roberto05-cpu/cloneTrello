...
const getBoardByIdController = async (req, res) => {
  try {
    ...
    // Compter les lists et cards pour ce board
    const listsCount = await listModel.countDocuments({ boardId });
    const cardsCount = await cardModel.countDocuments({ boardId });
